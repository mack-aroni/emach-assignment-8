...
# Function to generate two ellipsoid clusters with a given distance between them
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    # Shift the second cluster by the given distance
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        loss = -np.mean(np.log(model.predict_proba(X)[range(len(y)), y.astype(int)]))
